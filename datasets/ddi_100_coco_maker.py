...
class DDI100CocoMakingConfig(DatasetMakingConfig):
    ...
    dataset_root: Path = Path('/nfs/private/gilad/ocr_detection/data/raw/DDI_100_v1.3')
...
class DDI100CocoMaker(CocoDatasetMaker):
    ...
    def __post_init__(self):
        ...
        if self.config.blend:
            blend_ratios.extend(self.config.blend_ratios)
        if self.config.add_full_fusion:
            blend_ratios.append(-1)
    ...
    def __getitem__(self, idx: int) -> Optional[ImageWithAnnots]:
        ...
        xyxy_coords = list(map(lambda d: d['box'][[0, 1, 3, 2], ::-1], reverse_annots))
        ...
        if self.blend_ratios is None:
            ...
            if blend_ratio >= 0:
                ...
                final_im = origin_im.copy()
                final_im.paste(background_im, mask=Image.fromarray(~((np.array(origin_im) !=255).any(axis=2))))
                file_name = f'{file_name}_fuse'
